...
const StallDetailScreen = ({ route, navigation }) => {
  ...
  useEffect(() => {
    navigation.setOptions({ title: stallName });
    fetchStallData();
    fetchReviews();
  }, [stallId]);
  ...
  const fetchReviews = async () => {
    try {
      const res = await axios.get(`${API_BASE_URL}/api/stalls/${stallId}/reviews`);
      setReviews(res.data);
    } catch (err) {
  ...
  return (
    <ScrollView style={styles.container}>
            ...
            <View key={item._id || index} style={styles.menuItem}>
              <View style={styles.menuItemInfo}>
                ...
                <View style={styles.menuItemTags}>
                  {item.isSpicy && (
                    <Badge value="Spicy" badgeStyle={{ backgroundColor: '#e74c3c' }} textStyle={{ fontSize: 10 }} />
                  )}
                  {item.isSignature && (
                    <Badge value="Signature" badgeStyle={{ backgroundColor: '#f39c12' }} textStyle={{ fontSize: 10 }} />
                  )}
                </View>
              </View>
      ...
      <Card containerStyle={styles.menuCard}>
        ...
        {reviews.length === 0 ? (
          <Text style={styles.noMenuText}>No reviews yet</Text>
        ) : (
      ...
      <Button
        ...
        icon={{ name: 'shopping-cart', type: 'font-awesome', size: 16, color: 'white' }}
      />
    </ScrollView>
  );
};
